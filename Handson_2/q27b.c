...
int main() {
    ...
    printf("Waiting for the message (non-blocking)\n");
    if (msgrcv(msqid, &msg, MSGSZ, 0, IPC_NOWAIT) < 0) {
        if (errno == ENOMSG) {
            printf("No message available now\n");
        } else {
...
---------------Terminal 2(reciever)----------------
asutosh@asutosh-Nitro-AN515-43:~/Documents/OS/Handson_2$ cc 27b.c
asutosh@asutosh-Nitro-AN515-43:~/Documents/OS/Handson_2$./a.out
Waiting for the message (non-blocking)
